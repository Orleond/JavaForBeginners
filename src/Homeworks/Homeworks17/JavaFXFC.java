...
/**
 * Преобразуйте Swing-программу для сравнения файлов из упражнения 16.1
 * в приложение JavaFX. При этом воспользуйтесь предоставляемой в JavaFX
 * возможностью запускать события действий для кнопки
 * программными средствами. Это делается путем вызова метода fire() для
 * экземпляра кнопки. К примеру, если имеется экземпляр класса Button, который
 * вы назвали myButton, то для запуска события необходимо вызвать метод
 * myButton.fire(). Воспользуйтесь этим при реализации обработчиков
 * событий для текстовых полей, в которых хранятся имена сравниваемых
 * файлов. В тех случаях, когда пользователь нажимает клавишу <Enter> и
 * при этом фокус ввода находится в одном из указанных текстовых полей,
 * запускайте событие действия для кнопки Сравнить. После этого код
 * обработчика событий для кнопки Сравнить должен выполнить сравнение
 * файлов.
 */
